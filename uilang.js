document.addEventListener('DOMContentLoaded', function () {
  ...
  function f(a, b, c, d) {
    ...
    this.classBehavior =
      's' == b.charAt(b.length - 1) ? b.substring(0, b.length - 1) : b;
    this.classValue = '.' == c.charAt(0) ? c.substring(1, c.length) : c;
    this.targetSelector = d;
    this.createEventListener();
  }
  for (
    var g = document.getElementsByTagName('code'), h = g.length, e, k;
    h--;
  ...
  e &&
    ...
    (f.prototype.createEventListener = function () {
      function a(a) {
        switch (b.targetSelector) {
          ...
          default:
            for (
              var c = document.querySelectorAll(b.targetSelector), d = c.length;
              d--;

            )
              c.item(d).classList[b.classBehavior](b.classValue);
        }
      ...
      if (1 > d)
        throw Error(
          'There\'s no element matching your "' +
            b.clickSelector +
            '" CSS selector.'
        );
